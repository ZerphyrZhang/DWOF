...
uses
  windows, sysutils, syncobjs, classes, dialogs, FileCtrl, db;
...
{*************************}
  ...
  private
    ...
    FMsgLevel: Integer;
  public
    constructor Create(AStr: string; const ALevel: integer = 0);
    property LogMessage: string read FLogMsg;
    property MsgLevel: Integer read FMsgLevel;
  end;
{*************************}
...
{*************************}
  ...
  public
    ...
    procedure AddLog(AStr: string; const ALevel: integer = 0);
...
procedure TLogManager.AddLog(AStr: string; const ALevel: integer = 0);
begin
  LockQueue;
  try
    FLogMsgQueue.Add(TLogMessage.Create(AStr, ALevel));
  finally
    UnLockQueue;
  end;
...
procedure TLogManager.DelPreviousLogFiles(ADate: TDateTime; ADaysAgo: integer);
var
  TmpDate: TDateTime;
  FileName: string;
  IndDay: integer;
...
begin
  Flush(FLogFile);
  CloseFile(FLogFile);
  for i := 0 to FLogMsgQueue.Count - 1 do
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      self.FMsgLevel := TLogMessage(FLogMsgQueue.items[0]).MsgLevel;
...
begin
  ...
  if not FileExists(sTmpName) then
  begin
...
constructor TLogMessage.Create(AStr: string; const ALevel: integer = 0);
begin
  inherited create;
  FLogMsg := '[' + FormatDateTime('yyyy/mm/dd hh:nn:ss:zzz', now) + ']';
  FLogMsg := FLogMsg + '------> ' + AStr;
  FMsgLevel := ALevel;
end;
