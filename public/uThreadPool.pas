...
uses
  Windows, Classes, IdContext, IdCustomHTTPServer, ResUtils;
...
type
  ...
  // 线程初始化时触发的事件
  TProcessorThreadInitializing = procedure(Sender: TThreadsPool; aThread: TProcessorThread) of object;
  // 线程结束时触发的事件

  TProcessorThreadFinalizing = procedure(Sender: TThreadsPool; aThread: TProcessorThread) of object;
  // 线程处理请求时触发的事件
  ...
  TQueueEmpty = procedure(Sender: TThreadsPool; EmptyKind: TEmptyKind) of object;
  ...
  protected
    ...
    procedure DoProcessRequest(aDataObj: TWorkItem; aThread: TProcessorThread); virtual;
  ...
  public
    ...
    function AddRequest(aDataObject: TWorkItem; CheckForDoubles: Boolean = False): Boolean; overload;
    // 转换枚举类型的线程状态为字串类型
    function InfoText: string;
  published
    // 线程处理任务时触发的事件
...
type
  //日志记志函数
  TLogWriteProc = procedure(const Str: string; //日志
    LogID: Integer = 0; Level: Integer = 0 //Level = 0 - 跟踪信息, 10 - 致命错误
  );
...
function TThreadsPool.AddRequest(aDataObject: TWorkItem; CheckForDoubles: Boolean = False): Boolean;
var
  i: Integer;
begin
{$IFNDEF NOLOGS}
...
{$IFNDEF NOLOGS}
      WriteLog(Format('FThreads.Count (%d)<FThreadsMax(%d), AvgWait=%d', [FThreads.Count, FThreadsMax, AvgWait]), 4);
{$ENDIF}
...
procedure TThreadsPool.DoProcessRequest(aDataObj: TWorkItem; aThread: TProcessorThread);
begin
...
{$ENDIF}
            ...
            on e: Exception do
              WriteLog('OnProcessRequest for ' + FProcessingDataObject.TextForLog + #13#10'raise Exception: ' + e.Message, 8);
          end;
...
const
  ThreadStateNames: array[TThreadState] of string = ('tcsInitializing', 'tcsWaiting', 'tcsGetting', 'tcsProcessing', 'tcsProcessed', 'tcsTerminating', 'tcsCheckingDown');
begin
{$IFNDEF NOLOGS}
  Result := Format('%5d: %15s, AverageWaitingTime=%6d, AverageProcessingTime=%6d', [ThreadID, ThreadStateNames[FCurState], AverageWaitingTime, AverageProcessingTime]);
  case FCurState of
    tcsWaiting:
...
begin
  Result := Terminated or (FPool.ThreadDeadTimeout > 0) and (FCurState = tcsProcessing) and (GetTickCount - uProcessingStart > FPool.ThreadDeadTimeout);
  if Result then
...
begin
  ...
  // 则视为空闲。
  Result := (FCurState in [tcsWaiting, tcsCheckingDown]) and (AverageWaitingTime > 100) and (AverageWaitingTime * 2 > AverageProcessingTime);
